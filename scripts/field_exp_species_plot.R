...
c <- ggplot(data = summary_data, aes(x = treatment, y = mean_ulva_pt, colour = region)) + 
  ...
  labs(x = "", y = expression(paste(italic("Ulva"), " sp. (% cover)")), color = "Salinity region", title = "a") + 
...
ggsave("./figures/means_errors_ulva_chthamalus.png", plot = means_errors_main)
...
e <- ggplot(data = summary_data, aes(x = treatment, y = mean_balanus_no, colour = region)) + 
  ...
  labs(x = "", y = expression(paste(italic("Balanus glandula"), " (no.)")), fill = "Salinity region") + 
  my_theme + 
...
f <- ggplot(data = summary_data, aes(x = treatment, y = mean_mytilus_pt, colour = region)) + 
  ...
  labs(x = "", y = expression(paste(italic("Mytilus trossulus"), , " (no.)")), fill = "Salinity region") + 
  my_theme + 
  theme(axis.text.x=element_blank(),
        axis.ticks.x = element_blank())
...
g <- ggplot(data = summary_data, aes(x = treatment, y = mean_diatom_pt, colour = region)) + 
  ...
  labs(x = "", y = "Diatoms (% cover)", colour = "Salinity region") + 
  theme(axis.title = element_text(size = 12, colour = "grey30"), 
        panel.background = element_blank(), 
        panel.border = element_rect(fill = NA, colour = "grey30"), 
        panel.grid = element_blank(),
...
h <- ggplot(data = summary_data, aes(x = treatment, y = mean_masto_crust_pt, colour = region)) + 
  ...
  labs(x = "", y = expression(paste(italic("Petrocelis"), " (% cover)")), fill = "Salinity region") + 
  my_theme
...
i <- ggplot(data = summary_data, aes(x = treatment, y = mean_pyropia_pt, colour = region)) + 
  ...
  labs(x = "", y = expression(paste(italic("Pyropia"), " sp. (% cover)")), fill = "Salinity region") + 
  theme(axis.title.y = element_text(size = 12, colour = "grey30"), 
        axis.title.x = element_text(size = 14, colour = "grey30"),
        panel.background = element_blank(), 
        panel.border = element_rect(fill = NA, colour = "grey30"), 
        panel.grid = element_blank(),
        legend.key = element_blank(), 
...
j <- ggplot(data = summary_data, aes(x = treatment, y = mean_fucus_pt, colour = region)) + 
  ...
  labs(x = "", y = expression(paste(italic("Fucus distichus"), " (% cover)")), fill = "Salinity region") + 
  my_theme

xlab <- "Herbivore treatment"

multi_species_plot_SM <- e + f + g + h + i + j + grid.draw(grid::textGrob(xlab, x = 0.45, y = 0.025, gp = gpar(col = "black", fontsize = 14)))
