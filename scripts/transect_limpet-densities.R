...
# Lines 415-416: These lines read as, "The divergent pattern in limpet abundance 
# between regions over the summer likely reflects the influence of the low salinity 
# riverine output." There is no analysis of limpet densities to support this statement.
...
limpet_transect_plot <- ggplot(data = transect_limpets, aes(x = month, y = log(total_limpets + 1), color = site)) + 
  geom_boxplot() + 
  scale_colour_manual(values = c("orange", "orange", "orange", "steelblue", "steelblue", "steelblue"))  + 
  labs(c = "Month", y = "Total limpets", color = "Sites")

ggsave("./figures/limpet_transect_plot.jpg", plot = limpet_transect_plot)
