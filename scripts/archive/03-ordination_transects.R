...
survey_perma4 <- adonis2(decostand(survey_comm_data^(1/4),"range") ~ region + month + region*month, 
                         data = transect, 
                         permutations = perm, 
                         method = "bray")
survey_perma4
...
df <- data.frame(
  # treatments = c("salinity region", "month", "salinity region x month", "residuals", "total"),
                 df = survey_perma4$Df, 
                 SS = survey_perma4$SumsOfSqs, 
                 MS = survey_perma4$MeanSqs, 
                 `pseudo F` = survey_perma4$F, 
                 R2 = survey_perma4$R2, 
                 `p value` = survey_perma4$`Pr(>F)`)
...
# dist.wis <- vegdist(wisconsin(survey_comm_data^(1/2)), method = "bray")


## PSP: This is the correct way to run the test
...
levels(transect$region) <- c("Low", "High") 
...
survey_disp_df <- data.frame(Df = survey_disp_one.way$tab$Df, 
                                     SS = survey_disp_one.way$tab$`Sum Sq`,
                                     MS = survey_disp_one.way$tab$`Mean Sq`, 
                                     `F` = survey_disp_one.way$tab$`F`,
                                     N.Perm = survey_disp_one.way$tab$N.Perm,
                                     `Pr(>F)` = survey_disp_one.way$tab$`Pr(>F)`) %>% 
  mutate(across(where(is.numeric), round, 3)) %>% 
  rename(`Pr(>F)` = `Pr..F.`)

survey_disp_df[is.na(survey_disp_df)] <- ""

write_csv(survey_disp_df, here::here("tables", "survey_disp.csv"))
...
plot(dispersion_one.way, hull = FALSE, ellipse = TRUE, main = "",
     col = c("orange", "orange", "orange", "orange", 
             "steelblue", "steelblue", "steelblue", "steelblue")) # sd ellipse
title("A", adj = 0)
dev.off()
